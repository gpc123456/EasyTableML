...
class EasyTableMLRegression():
    ...
    def base_auto_parameter(self,
        ...
        for i, (name, model) in enumerate(model_list.items()):
            print('Note(Auto Parameter):', 'We are currently searching the ',
                  i + 1, " model's beat parameter, ", 'model name: ', name)
            print('Plase Wait...')
            ...
            joblib.dump(grid_search.best_estimator_,
                        os.path.join('models', 'AutoML', name + '.pkl'))
            print('Best model will be saved in:',
                  os.path.join('models', 'AutoML', name + '.pkl'))
    ...
    def train(self,
        ...
        if train_type == 'base':
            if auto_parameter == False:
                for i, (name, model) in enumerate(model_list.items()):
                    print('Note:', 'We are currently training the ', i + 1,
                ...
                model_list = self.base_auto_parameter(
                    model_list,
                    x_train,
                    y_train,
        ...
        if train_type == 'meta':
            if auto_parameter == False:
                ...
                print('Plase Wait...')
                meta_learner = StackingRegressor(estimators=list(
                    model_list.items()),
                                                 final_estimator=meta_model,
                                                 n_jobs=-1)
                meta_learner.fit(x_train, y_train)
                joblib.dump(meta_learner,
                            os.path.join('models', 'meta_learner.pkl'))
            else:
                if auto_custom_parameters == None:
                    meta_learner = StackingRegressor(
                        estimators=list(model_list.items()),
                        final_estimator=MLPRegressor(hidden_layer_sizes=(30,
                                                                         100,
                                                                         10),
                                                     max_iter=5000,
                                                     alpha=0.1),
                    ...
                    grid_search = GridSearchCV(StackingRegressor(
                        estimators=list(model_list.items()), cv=5),
                ...
                joblib.dump(
                    meta_learner,
                    os.path.join('models', 'AutoML', 'meta_learner.pkl'))
            return meta_learner
    ...
    def load_base_model(self, model_list, is_auto_parameter=False):
        if is_auto_parameter == False:
            for i in range(len(model_list)):
                ...
                name = model_ll[i][0]
                print('Note:', 'Load model ', i + 1, ' ,', 'model name: ',
                      name)
                model_list[name] = joblib.load(
                    os.path.join('models', name + '.pkl'))
            print('Load Done!')
        else:
            for i in range(len(model_list)):
                model_ll = list(model_list.items())
    ...
    def estimate_base(self,
        ...
        _, result_score_train = self.estimate(model_list, x_train, y_train,
                                              details)

        print('Estimating Valid/Test Set...')
        P, result_score_valid = self.estimate(model_list, x_valid, y_valid,
                                              details)
        ...
        if (len(overfit) > 0):
            print(
                'The fellowing models may have overfitting, please consider modifying them.'
            )
            print('Train score - Valid score:')
            print(overfit)
        if (len(low_score) > 0):
            print(
                'The fellowing models have lower scores, please consider removing them.'
            )
            print(low_score)
    ...
    def estimate_meta(self,
                      ...
                      details=0):
        print('Estimating Train Set...')
        _, result_score_train = self.estimate({'meta_model': meta_model},
                                              x_train, y_train, details)

        print('Estimating Valid/Test Set...')
        _, result_score_valid = self.estimate({'meta_model': meta_model},
                                              x_valid, y_valid, details)
        ...
        if (len(overfit) > 0):
            print(
                'The model may have overfitting, please consider modifying it.'
            )
            print('Train score - Valid score:')
            print(overfit)
        if (len(low_score) > 0):
